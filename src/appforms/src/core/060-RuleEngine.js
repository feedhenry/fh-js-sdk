...
function rulesEngine(formDef) {
  ...
  var asyncLoader = module.exports;
  // async has updated this, now save in our var, to that it can be returned from our dummy require
  function require() {
    return asyncLoader;
  }
  /* End of infix file */
 ...
 */
    ...
    var formsRulesEngine = function (formDef) {
      ...
      var submissionRequiredFieldsMap = {};
      // map to hold the status of the required fields per submission
      ...
      function buildSubmissionFieldsMap(cb) {
        submissionRequiredFieldsMap = JSON.parse(JSON.stringify(requiredFieldMap));
        // clone the map for use with this submission
        submissionFieldsMap = {};
        // start with empty map, rulesEngine can be called with multiple submissions
        ...
        async.each(submission.formFields, function (formField, cb) {
          if (!formField.fieldId)
            return cb(new Error('No fieldId in this submission entry: ' + util.inspect(formField)));
          ...
          getPreviousFieldValues(submittedField, previousSubmission, function (err, previousFieldValues) {
            ...
            getFieldValidationStatus(submittedField, fieldDef, previousFieldValues, function (err, fieldRes) {
              ...
              if (!fieldRes.valid) {
                ...
                // indicate invalid form if any fields invalid
          ...
          if (!submissionRequiredFieldsMap[requiredFieldId].submitted) {
            isFieldVisible(requiredFieldId, true, function (err, visible) {
              if (err)
                return cb(err);
              if (visible) {
                // we only care about required fields if they are visible
 ...
 */
      function validateFieldValue(fieldId, inputValue, valueIndex, cb) {
        ...
        init(function (err) {
          ...
          var required = false;
          if (fieldDefinition.repeating && fieldDefinition.fieldOptions && fieldDefinition.fieldOptions.definition && fieldDefinition.fieldOptions.definition.minRepeat) {
            ...
            async.map(submittedField.fieldValues, function (fieldValue, cb) {
              if ('undefined' === typeof fieldValue || null === fieldValue) {
                ...
                validator(fieldValue, fieldDefinition, previousFieldValues, function (validationError) {
                  ...
                  if (submissionRequiredFieldsMap[fieldDefinition._id]) {
                    // set to true if at least one value
      ...
      function validatorFile(fieldValue, fieldDefinition, previousFieldValues, cb) {
        ...
        async.each(keyTypes, function (keyType, cb) {
          ...
          if (err)
            return cb(err);
          if (fieldValue.hashName.indexOf('filePlaceHolder') > -1) {
            //TODO abstract out to config
            return cb();
        ...
        async.each(rules, function (rule, cbRule) {
          ...
          async.each(rule.ruleConditionalStatements, function (ruleConditionalStatement, cbPredicates) {
            ...
            function rulesPassed(condition, passed, queries) {
              return condition === 'and' && passed.length == queries.length || condition === 'or' && passed.length > 0;  // "or" condition - only one rule must pass
            }
      ...
      function isPageVisible(pageId, cb) {
        init(function (err) {
          if (err)
            return cb(err);
          if (isPageRuleSubject(pageId)) {
            // if the page is the target of a rule
        ...
        init(function (err) {
          ...
          async.waterfall([
            ...
            function testField(pageVisible, cb) {
              if (!pageVisible) {
                // if page containing field is not visible then don't need to check field
                return cb(undefined, false);
              }
              if (isFieldRuleSubject(fieldId)) {
                // If the field is the subject of a rule it may have been hidden
